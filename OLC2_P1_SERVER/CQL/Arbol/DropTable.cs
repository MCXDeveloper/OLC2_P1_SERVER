...
public class DropTable : Instruccion
{
    ...
    public object Ejecutar(Entorno ent)
    {
        ...
        if (!CQL.BaseDatosEnUso.Equals(String.Empty))
        {
            ...
            {
                Error.AgregarError("Semántico", "[ALTER_TABLE]", "Error.  La tabla especificada '" + NombreTabla + "' no existe en la base de datos actual, por lo tanto, no se puede eliminar.", fila, columna);
            }
        }
        else
        {
            Error.AgregarError("Semántico", "[ALTER_TABLE]", "Error.  No se puede eliminar una tabla si no se ha especificado la base de datos a utilizar.", fila, columna);
        }
