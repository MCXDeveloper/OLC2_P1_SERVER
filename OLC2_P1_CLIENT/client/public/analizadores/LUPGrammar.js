...
case 29:return 5;
break;
case 30: console.log('Error léxico: ' + yy_.yytext + ', en la linea: ' + yy_.yylloc.first_line + ', en la columna: ' + yy_.yylloc.first_column); 
break;
}
},
rules: [/^(?:[ \r\t]+)/i,/^(?:\n)/i,/^(?:\[\+ERROR\])/i,/^(?:\[-ERROR\])/i,/^(?:\[\+DATABASES\])/i,/^(?:\[-DATABASES\])/i,/^(?:\[\+DATABASE\])/i,/^(?:\[-DATABASE\])/i,/^(?:\[\+TABLES\])/i,/^(?:\[-TABLES\])/i,/^(?:\[\+TABLE\])/i,/^(?:\[-TABLE\])/i,/^(?:\[\+TYPES\])/i,/^(?:\[-TYPES\])/i,/^(?:\[\+TYPEX\])/i,/^(?:\[-TYPEX\])/i,/^(?:\[\+NAME\][^\[]*\[-NAME\])/i,/^(?:\[\+COLUMNS\][^\[]*\[-COLUMNS\])/i,/^(?:\[\+ATTRIBUTES\][^\[]*\[-ATTRIBUTES\])/i,/^(?:\[\+PROCEDURES\][^\[]*\[-PROCEDURES\])/i,/^(?:\[\+LINE\](.*)\[-LINE\])/i,/^(?:\[\+COLUMN\](.*)\[-COLUMN\])/i,/^(?:\[\+LOCATION\](.*)\[-LOCATION\])/i,/^(?:\[\+TYPE\][^\[]*\[-TYPE\])/i,/^(?:\[\+DESC\](.*)\[-DESC\])/i,/^(?:\[\+DATA\][^\[]*\[-DATA\])/i,/^(?:\[\+MESSAGE\][^\[]*\[-MESSAGE\])/i,/^(?:\[\+LOGIN\](.*)\[-LOGIN\])/i,/^(?:\[\+LOGOUT\](.*)\[-LOGOUT\])/i,/^(?:$)/i,/^(?:.)/i],
